...
@Component({
  ...
  templateUrl: './asentamientos.page.html',
  styleUrls: ['./asentamientos.page.scss'],
})
export class AsentamientosPage {
  riesgos: any;
  subscription: any;
  ...
  async deleteRiesgo(id: string) {
    ...
    await this.firestore.doc("asentamientos/" + id).delete();
  ...
  addRiesgo() {
    if (aux == 1) {
      this.router.navigateByUrl("/add-asentamientos");
    } else {
      this.alertController.create({
        header: 'Alerta',
        subHeader: 'Permiso Denegado',
  ...
  editRiesgo(id: string) {
    if (aux == 1) {
      this.router.navigateByUrl("/edit-asentamientos/"+id);
    } else {
      this.alertController.create({
        header: 'Alerta',
        subHeader: 'Permiso Denegado',

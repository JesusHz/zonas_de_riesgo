...
@Component({
  selector: 'app-inviernos',
  templateUrl: './inviernos.page.html',
  styleUrls: ['./inviernos.page.scss'],
})
export class InviernosPage {
  riesgos: any;
  subscription: any;
  ...
  async deleteRiesgo(id: string) {
    ...
    await this.firestore.doc("inviernos/" + id).delete();
  ...
  addRiesgo() {
    if (aux == 1) {
      this.router.navigateByUrl("/add-inviernos");
    } else {
      this.alertController.create({
        header: 'Alerta',
        subHeader: 'Permiso Denegado',
  ...
  editRiesgo(id: string) {
    if (aux == 1) {
      this.router.navigateByUrl("/edit-inviernos/"+id);
    } else {
      this.alertController.create({
        header: 'Alerta',
        subHeader: 'Permiso Denegado',

...
@Component({
  selector: 'app-mas-extensos',
  templateUrl: './mas-extensos.page.html',
  styleUrls: ['./mas-extensos.page.scss'],
})
export class MasExtensosPage {
  extensos: any;
  subscription: any;
  ...
  async deleteExtenso(id: string) {
    ...
    await this.firestore.doc("mas-extensos/" + id).delete();
  ...
  addExtenso() {
    if (aux == 1) {
      this.router.navigateByUrl("/add-mas-extensos");
    } else {
      this.alertController.create({
        header: 'Alerta',
        subHeader: 'Permiso Denegado',
  ...
  editExtenso(id: string) {
    if (aux == 1) {
      this.router.navigateByUrl("/edit-mas-extensos/"+id);
    } else {
      this.alertController.create({
        header: 'Alerta',
        subHeader: 'Permiso Denegado',

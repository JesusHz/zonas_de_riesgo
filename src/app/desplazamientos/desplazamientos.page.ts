...
@Component({
  ...
  templateUrl: './desplazamientos.page.html',
  styleUrls: ['./desplazamientos.page.scss'],
})
export class DesplazamientosPage {
  riesgos: any;
  subscription: any;
  ...
  async deleteRiesgo(id: string) {
    ...
    await this.firestore.doc("desplazamientos/" + id).delete();
  ...
  addRiesgo() {
    if (aux == 1) {
      this.router.navigateByUrl("/add-desplazamientos");
    } else {
      this.alertController.create({
        header: 'Alerta',
        subHeader: 'Permiso Denegado',
  ...
  editRiesgo(id: string) {
    if (aux == 1) {
      this.router.navigateByUrl("/edit-desplazamientos/"+id);
    } else {
      this.alertController.create({
        header: 'Alerta',
        subHeader: 'Permiso Denegado',

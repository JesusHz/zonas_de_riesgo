...
@Component({
  ...
  templateUrl: './fracturas.page.html',
  styleUrls: ['./fracturas.page.scss'],
})
export class FracturasPage {
  riesgos: any;
  subscription: any;
  ...
  async deleteRiesgo(id: string) {
    ...
    await this.firestore.doc("fracturas/" + id).delete();
  ...
  addRiesgo() {
    if (aux == 1) {
      this.router.navigateByUrl("/add-fracturas");
    } else {
      this.alertController.create({
        header: 'Alerta',
        subHeader: 'Permiso Denegado',
  ...
  editRiesgo(id: string) {
    if (aux == 1) {
      this.router.navigateByUrl("/edit-fracturas/"+id);
    } else {
      this.alertController.create({
        header: 'Alerta',
        subHeader: 'Permiso Denegado',

...
@Component({
  selector: 'app-edit-erupciones',
  templateUrl: './edit-erupciones.page.html',
  styleUrls: ['./edit-erupciones.page.scss'],
})
export class EditErupcionesPage implements OnInit {
  riesgo = {} as Riesgo;
  id: any;
  ...
  async updateRiesgo(riesgo: Riesgo) {
    if (this.formValidation()) {
      ...
      try {
        await this.firestore.doc("erupciones/" + this.id).update(riesgo);
      } catch (e) {
        this.showToast(e);
      }
      ...
      this.navCtrl.navigateRoot("erupciones");
    }
  }
